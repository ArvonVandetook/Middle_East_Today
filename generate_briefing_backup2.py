...
HAARETZ_HOME_URL = "https://www.haaretz.com/"
HAARETZ_MAX_ARTICLES = 20
...
def crawl_jadaliyya():
    ...
    with sync_playwright() as p:
        ...
        try:
            print("[JAD] loading homepage via Playwright")

            page.goto(JADALIYYA_SEED_URL, timeout=30000)
            ...
            links = list(set(links))[:JAD_MAX_ARTICLES]

            print(f"[JAD] article links found: {len(links)}")

        except Exception as e:
            print("[JAD ERROR - load]", e)
            browser.close()
            return []
...
# ---------------------------
# HAARETZ (HOMEPAGE CRAWLER)
# ---------------------------

def is_valid_haaretz_link(url):
    if not url:
        return False
    ...
    if "haaretz.com" not in full:
        return False

    if "ty-article" not in path:
        return False

    # Skip obvious junk / non-targets
    bad_parts = [
        "ty-article-podcast",
        "photo-blog",
        "haquiz",
    ...
    if any(b in full for b in bad_parts):
        return False

    # Skip live blog item fragments; keep the main live page only
    if "liveblogitemid=" in full or "#" in full:
        return False
...
def crawl_haaretz():
    ...
    try:
        ...
        r = session.get(HAARETZ_HOME_URL, timeout=15)
        ...
        for a in soup.find_all("a", href=True):
            url = urljoin(HAARETZ_HOME_URL, a["href"])
            title = clean_text(a.get_text(" ", strip=True))

            if not is_valid_haaretz_link(url):
                continue
        ...
        print(f"[HAARETZ] candidate links found: {len(candidates)}")

        for url, fallback_title in candidates[:HAARETZ_MAX_ARTICLES]:
            try:
                page = session.get(url, timeout=10)
                psoup = BeautifulSoup(page.text, "lxml")
                ...
                summary = truncate(clean_html(" ".join(body_paras))) if body_paras else fallback_title
    ...
    print(f"[HAARETZ] crawled: {len(results)}")
    return results
...
def fetch_all():
    ...
    haaretz = crawl_haaretz()
    print(f"[HAARETZ] before filter: {len(haaretz)}")
    haaretz = [a for a in haaretz if is_relevant(a)]
    print(f"[HAARETZ] after relevance: {len(haaretz)}")
    haaretz = [a for a in haaretz if not is_low_signal(a)]
    print(f"[HAARETZ] after low-signal: {len(haaretz)}")
    items.extend(haaretz)
    ...
    jad = crawl_jadaliyya()
    print(f"[JAD] before filter: {len(jad)}")
    jad = [a for a in jad if is_relevant(a)]
    print(f"[JAD] after relevance: {len(jad)}")
    jad = [a for a in jad if not is_low_signal(a)]
    print(f"[JAD] after low-signal: {len(jad)}")
    items.extend(jad)
...
def build():
    ...
    jad_scores = [a["importance"] for a in enriched if a["source"] == "Jadaliyya"]
    if jad_scores:
        print(f"[JAD] avg importance: {sum(jad_scores)/len(jad_scores):.1f}")
    ...
    final_jad = sum(1 for a in events + regional + deep if a["source"] == "Jadaliyya")

    print(f"[AMWAJ] final selected: {final_amwaj}")
    print(f"[JAD] final selected: {final_jad}")
